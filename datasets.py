...
from configs.configs_global import DATA_DIR, DEVICE
...
def model_representation(model_list, handles_list, data_loader, max_num_data=1000, record_input=False):
    r"""
    ...
    :Input:
             ...
             record_input - Bool, True if need to consider the input as activation, False not consider
    ...
    with torch.no_grad():
        for i, data in enumerate(data_loader):
            inputs, _ = data
            if record_input:
                inp = inputs.clone().detach().cpu().numpy()
                input_data.append(inp.reshape(data_loader.batch_size, -1))
    ...
    activation_list = []
    if record_input:
        activation = {'input': np.concatenate(input_data, axis=0)}
...
if __name__ == '__main__':
    ...
    activation = model_representation([model,], [handles,], data_loader, max_num_data=20, record_input=True)
